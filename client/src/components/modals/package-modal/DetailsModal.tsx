...
const DetailsModal: React.FC<DetailsModalProps> = ({onClose, openEditModal, errorMsg, refresh, packageId}) => {
  ...
  <div>
    ...
    <div className='z-0 absolute top-0 left-0 bg-[rgba(0,0,0,0.5)] w-[100vw] h-full overflow-hidden backdrop-blur-sm animate-zoom-in xs:max-sm:h-[63rem]'>
    <div className='flex justify-center align-center my-20 xl:max-2xl:my-16'>
        <div className="w-[75vw] h-[80vh] bg-white p-10 rounded-xl xs:max-sm:p-6 xs:max-sm:w-[90vw] xs:max-sm:h-[70vh] xl:max-2xl:h-[83vh] xl:max-2xl:p-6">
          {deleteModal && (
      ...
      <div className='grid grid-cols-2 h-[5vh] border-b-2 border-black '> {/*this is the header for the modal*/}
          ...
          <div className='flex justify-end mb-4'>
            <button onClick={onClose} className='flex items-center text-3xl xs:max-sm:text-[1.5em] xl:max-2xl:text-2xl '>
      ...
      <div className='h-[60vh] my-5 border-b-2 border-solid border-[#000000] flex justify-center pt-[5%]'>
        <p className='text-center'>
        <GenSpinner/></p><p className='pt-[5%] text-center animate-pulse'>Loading...</p>
      </div>
      :
      
      <div className="grid grid-cols-2 h-[60vh] my-5 border-b-2 border-solid border-[#000000] xs:max-sm:h-[53vh]">
            <div>
              <div className='h-[40vh] text-xl xs:max-sm:text-[0.8em] xs:max-sm:h-[50vh] xl:max-2xl:h-[20vh] xl:max-2xl:text-[0.7em] '>
                <table className='border-separate border-spacing-x-8 border-spacing-y-2 xs:max-sm:border-spacing-y-[0] xs:max-sm:border-spacing-x-2 xl:max-2xl:border-spacing-y-[0] xl:max-2xl:ml-[3%]'>
                  <tr >
                  ...
                  <tr>
                    <td><b>Available From: </b></td><td className="flex">{dateStart.split('T')[0]} at {timeStart}</td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    <td><b>Tags: </b></td><td><div className=' w-[20vw] overflow-x-hidden hover:overflow-x-auto h-[5vh]'>
                      {tags.map((tag:string, index) => (<button className=' w-auto text-[0.8em] mx-1 border rounded-lg px-2 hover:bg-blue-200 duration-200 xl:max-2xl:text-[0.8em]' key={index}>{tag}</button>))}  </div></td>
                  </tr>
                  <tr>
                    <td><b>Visibility: </b></td><td><div className={` text-center font-bold rounded-2xl 
                    ${visibility === "PUBLISHED"? ' w-[50%] bg-green-600 text-white xl:max-2xl:w-[50%]': 'bg-blue-200 text-blue-500 xl:max-2xl:w-[50%]'}`}>{visibility}</div></td>
                  </tr>
                  <tr className='h-[10]'>
                  ...
                  <div className='text-[0.9em] border px-2 h-[10vh] w-[25vw] overflow-y-hidden  xs:max-sm:w-[100%]'>
                    <textarea className='w-[100%] h-[100%] bg-white overflow-y-hidden hover:overflow-y-visible  resize-none' disabled value={description}></textarea></div></td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                  <td colSpan={2}>
                    <div className='border px-4 overflow-y-auto h-[10vh] w-[25vw] xs:max-sm:w-[100%] xl:max-2xl:h-[8vh]'>
                    <ul className='list-disc ml-[6%] '>
                    {items.map((item, index) => (
                                            <li key={index} className='text-[0.9em]'>{item}</li>
                                        ))}      
                    </ul>
                    </div>
                    </td>
            ...
            <div className='flex justify-end items-center h-[3vh]'>{/*This is the footer*/}
                <button className='w-[8vw] h-[4vh] mx-5 rounded-md text-[1.1rem] bg-[#e14f4c] flex items-center justify-center xs:max-sm:w-[25vw] xs:max-sm:text-[0.8em] xl:max-2xl:text-[0.8em]
                ...
                <button className='w-[8vw] h-[4vh] text-[1.1rem] bg-[#efb953] mx-5 rounded-md flex items-center justify-center xs:max-sm:w-[25vw] xs:max-sm:text-[0.8em] xl:max-2xl:text-[0.8em]
