	...
	const fetchGraphInfo = async () => {
		try {
			const responseAccounts = await axios.get(
				`${config.API}/user/retrieve_accounts`,
				{
					params: {
						year: "2023",
					},
				...
				<div className="w-[65%] p-[1%] text-center">
					<div className="p-[3%] h-[100%] rounded-3xl bg-gradient-to-t from-[#930a08] to-[#600403] grid-cols-2 grid-rows-2 gap-1 flex">
						<div className="w-[33%] mt-[-1%] xs:max-sm:w-[50%] xs:max-sm:h-[35%] xs:max-sm:mt-[13%] xl:max-2xl:mt-[1%]">
							<p className="text-white mt-[-5%] text-[8em] font-bold flex-col xs:max-sm:text-[3em]  xl:max-2xl:text-[4em]">
								35
							</p>
							<p className="text-white text-[2em] flex-col mt-[-10%] xs:max-sm:text-[0.9em] xs:max-sm:ml-[2%] xl:max-2xl:text-[2em]">
								Merchants
							</p>
						</div>
						<div className="w-[33%] mt-[-1%] xs:max-sm:w-[50%] xs:max-sm:h-[35%] xs:max-sm:mt-[13%] xl:max-2xl:mt-[1%]">
							...
							<p className="text-white text-[2em] flex-col mt-[-10%] xs:max-sm:text-[0.9em] xs:max-sm:ml-[20%] xl:max-2xl:text-[2em]">
								Users
							</p>
						</div>
			...
			<div className="bg-[#F3F3F3] h-[30vh] flex overflow-auto xs:max-sm:h-[35vh]">
				<div className="w-[75%] m-[1%] text-center bg-white rounded-3xl flex-col pt-0 p-[1%] overflow-auto">
					<div className="text-left border-b-2 border-black">
						<p className="font-bold text-[1.5em] xs:max-sm:text-[1.2em] xl:max-2xl:text-[1em]">
							Recent Sign Ups
							<br />
						</p>
					</div>
