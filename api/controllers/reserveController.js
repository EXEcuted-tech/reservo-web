	...
	try {
		db.query(insertQuery, data, (err, result) => {
			if (err) {
				...
				return res
					.status(500)
					.json({ status: 500, success: false, error: "Error inserting data" });
			}
		...
		return res
			.status(500)
			.json({ status: 500, success: false, error: "An error occurred" });
	}
};
...
const updateReserve = (req, res) => {
	...
	const updateQuery =
		"UPDATE reservation SET res_date=?,res_time=?,res_location=?,party_size=?,settings=?,additional_details=?,account_id=?,merchant_id=?,sched_id=?,package_id=?,payment_id=?,inventory_id=?,status=? WHERE reservation_id=?";
	...
	db.query(updateQuery, data, (err, result) => {
		if (err) {
			...
			return res
				.status(500)
				.json({ status: 500, success: false, error: "Error updating data" });
		}
		...
		if (result.affectedRows > 0) {
			...
			return res
				.status(500)
				.json({ status: 500, success: false, error: "Updating record failed" });
		}
	});
};
...
const retrieveCountByParams = (req, res) => {
	...
	const retrieveSpecific =
		"SELECT COUNT(*) as count FROM reservation WHERE ?? = ?";
...
const retrieveBookingsByMonth = (req, res) => {
	...
	const retrieveYear =
		"SELECT YEAR(date_received) as year, MONTH(date_received) as month, COUNT(*) as books FROM reservation WHERE YEAR(date_received) = ? AND merchant_id = ? GROUP BY reservation.date_received;";
...
const retrieveCountByTwoParams = (req, res) => {
	...
	const retrieveSpecific =
		"SELECT COUNT(*) as count FROM reservation WHERE ?? = ? AND ?? = ?";
...
const retrieveCountByThreeParams = (req, res) => {
	...
	const retrieveSpecific =
		"SELECT COUNT(*) as count FROM reservation WHERE ?? = ? AND ?? = ? AND ??  = ?";
...
const deleteReserve = (req, res) => {
	...
	db.query(deleteQuery, res_id, (err, result) => {
		if (err) {
			...
			return res
				.status(500)
				.json({ status: 500, success: false, error: "Error deleting records" });
		} else {
			return res.status(200).json({
				status: 200,
				success: true,
